...
const sentinel = new Operator(OperatorType.Sentinel, false);

interface ParseResult {
    operationType: OperationType, errorType: ParseErrorType
}
...
export class Expression extends AstNodeImpl {
 ...
 // https://en.wikipedia.org/wiki/Shunting-yard_algorithm
  ...
  private parseExpression(context: ParseContext) {
      ...
      switch (ct.tokenType) {
        ...
        case TokenType.Symbol:
          ...
          case TokenType.OpenCurly:
            result = this.handleSet(context);
            currentOperationType = result.operationType;
            errorType = result.errorType;
            break;

          case TokenType.OpenBracket:
            result = this.handleSquareBrackets(context);
            currentOperationType = result.operationType;
            errorType = result.errorType;
            break;
...
private isConsistentOperationSequence(context: ParseContext, currentOperationType: OperationType): boolean {
  ...
  if ((this._previousOperationType === OperationType.Function && currentOperationType == OperationType.Operand) ||
      ...
      switch (currentOperationType) {
          case OperationType.Operand:
              ...
              context.addError(new ParseError(ParseErrorType.OperatorExpected, ErrorLocation.Token, this.getOperandErrorRange(context)));
              break;
          ...
          default:
              ...
              context.addError(new ParseError(ParseErrorType.OperandExpected, ErrorLocation.Token, this.getOperatorErrorRange(context)));
              break;
      }
  ...
  if (this._previousOperationType == OperationType.UnaryOperator && currentOperationType == OperationType.BinaryOperator) {
      // unary followed by binary doesn't make sense 
      context.addError(new ParseError(ParseErrorType.SymbolExpected, ErrorLocation.Token, this.getOperatorErrorRange(context)));
      return false;
  }
  if (this._previousOperationType == OperationType.BinaryOperator && currentOperationType == OperationType.EndOfExpression) {
      // missing list selector: z$ }
      context.addError(new ParseError(ParseErrorType.OperandExpected, ErrorLocation.Token, this.getErrorRange(context)));
      return false;
  }
...
private handleSet(context: ParseContext): ParseResult {
    var set = new Set();
    set.parse(context, undefined);
    this._operands.push(set);
    return {operationType: OperationType.Operand, errorType: ParseErrorType.None};
}

private handleOperator(context: ParseContext): ParseResult {
    const currentOperator = new Operator(firstInExpression: _operands.Count == 0);
    currentOperator.parse(context, null);
    const isUnary = currentOperator.unary;
    let operationType = isUnary ? OperationType.UnaryOperator : OperationType.BinaryOperator;

    return this.handleOperatorPrecedence(context, currentOperator);
}
...
private processHigherPrecendenceOperators(context: ParseContext, currentOperator: Operator): ParseErrorType {
    ...
    let errorType = ParseErrorType.None;
    let associativity = currentOperator.associativity;
...
// Constructs AST node from operator (root) and one or two operands.
  ...
  // and 'c' and 'a*b' as its child nodes.
       ...
       private makeNode(context: ParseContext): ParseErrorType {
        var operatorNode = this._operators.pop()!;

        var rightOperand = this.safeGetOperand(operatorNode);
        if (!rightOperand) {
            // Oddly, no operands
        ...
        if (operatorNode.unary) {
            ...
            var leftOperand = this.safeGetOperand(operatorNode);
            if (leftOperand == null) {
                context.addError(new ParseError(ParseErrorType.OperandExpected, ErrorLocation.Token, this.gGetIndexerOrFunctionErrorRange(context, operatorNode)));
                return ParseErrorType.OperandExpected;
            }
