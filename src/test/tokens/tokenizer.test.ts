...
import { A32Set, TokenSubType, TokenType } from '../../tokens/definitions';
...
test('=X4', () => {
  ...
  TokenTest.verifyTokenTypes(actual, [TokenType.Symbol, TokenType.Operator, TokenType.Symbol]);
  TokenTest.verifyTokenSubTypes(actual, [TokenSubType.None, TokenSubType.None, TokenSubType.Register]);
});
...
test('Hash comment', () => {
  const actual = TokenTest.tokenizeToArray(' # comment');
  TokenTest.verifyTokenTypes(actual, [TokenType.LineComment]);
});
